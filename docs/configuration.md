...
Herdr workspaces are derived from `FM_HOME`: the primary home uses `Atlas`, and a secondmate home with a readable, non-empty `.fm-secondmate-home` marker uses `Themis-<secondmate-id>`.
