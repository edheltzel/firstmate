...
- OMP uses that Pi-compatible tracked extension only through an explicit `-e` path, preserving a distinct adapter identity and load contract.
...
OMP's callback comes from the explicitly loaded Pi-compatible extension rather than Pi's trusted project auto-discovery.
...
Pi, pi-signed, and OMP keep the latch across internal tool turns and clear it only when the generated follow-up settles or delivery fails.
