...
At the time of this scout, `bin/fm-bootstrap.sh:457-460` rewrote a `dead` verdict to `unknown` unless the meta's `harness=` named claude, codex, opencode, pi, or grok. The respawn path was therefore gated **twice** - once on the backend having a verified classifier, once on the harness being a verified adapter. No stated reason appeared at the code site; it was consistent with the never-respawn-on-doubt argument at `:429-433`. OMP was verified later; current adapter facts are owned by `.agents/skills/harness-adapters/SKILL.md`.
