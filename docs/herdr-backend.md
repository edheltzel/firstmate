...
With the optional projection disabled, attach to the selected `HERDR_SESSION` and switch to the owning project's or secondmate's workspace to see its tasks as tabs in one tab bar.
...
Every newly projected child created by a primary or secondmate home is inserted as a top-level space immediately after its owning project Fleet or Archon parent contiguous child block when Herdr protocol 16 `workspace.move` and `python3` are available.
...
The pane report uses source `firstmate`, sets a title derived deterministically from the task id plus harness, and sets a `<Fleet display name>-Fleet-{Number}` display agent name plus the `fm_task`, `fm_project`, and `fm_harness` tokens whose values Firstmate already knows.
...
Display agent numbers are the lowest free positive integer in the target workspace, starting at `<Fleet display name>-Fleet-1`.
The allocator reads the target pane and workspace pane list while holding a shared session/workspace lock, so a live pane keeps its number and a number is reused only after its prior pane is gone.
An existing non-empty pane display name is left untouched because the reporter does not claim names it did not set.
The same display-name path is used for workers created from a secondmate home; the secondmate workspace label remains `Archon`-based.
...
The reporter never passes `--agent`, `--clear-display-agent`, another clear flag, or an agent-authority command.
It passes `--display-agent` only for a newly created pane whose current display name is empty, so it does not clear, claim, or overwrite an existing Herdr agent name, and it does not author semantic `idle`, `working`, `blocked`, or `done` state.
...
## Agent display-name scope evidence (2026-07-29)

The installed Herdr version was `0.7.5-preview.2026-07-21-0f10e1453a7f`, protocol 17.
The live schema exposed pane `display_agent` and `clear_display_agent` fields and the `pane.report_metadata` method.
The installed source stores `display_agent` in `AgentMetadata` on each `TerminalState` (`~/.claude/skills/herdr/src/terminal/metadata.rs`), resolves a pane id before applying the report (`~/.claude/skills/herdr/src/app/api/panes.rs`), and exposes the `--display-agent` flag (`~/.claude/skills/herdr/src/cli/pane.rs`), with no session-global name registry.

The real experiment used only the isolated lab session generated by `bin/fm-herdr-lab.sh`:

```sh
HERDR_LAB_HELPER='/Users/ed/Developer/Atlas/Themis/bin/fm-herdr-lab.sh'
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name herdr-agent-display-names-n2)
trap '"$HERDR_LAB_HELPER" teardown "$HERDR_LAB_SESSION"' EXIT
"$HERDR_LAB_HELPER" provision "$HERDR_LAB_SESSION"
"$HERDR_LAB_HELPER" run "$HERDR_LAB_SESSION" workspace create --cwd /tmp --label Scope-A --no-focus
"$HERDR_LAB_HELPER" run "$HERDR_LAB_SESSION" workspace create --cwd /tmp --label Scope-B --no-focus
"$HERDR_LAB_HELPER" run "$HERDR_LAB_SESSION" pane report-metadata w1:p1 --source user:firstmate-test --display-agent Scope-A-Fleet-1
"$HERDR_LAB_HELPER" run "$HERDR_LAB_SESSION" pane report-metadata w2:p1 --source user:firstmate-test --display-agent Scope-B-Fleet-1
"$HERDR_LAB_HELPER" run "$HERDR_LAB_SESSION" pane get w1:p1
"$HERDR_LAB_HELPER" run "$HERDR_LAB_SESSION" pane get w2:p1
```

Both pane reads returned their own workspace id and retained their own Fleet-prefixed display name.
Therefore `display_agent` is pane-local and number allocation is scoped to each workspace. The captain-selected format nevertheless always includes the Fleet display name, independent of that scope result.
The lab helper teardown verified the default session fleet-state tripwire was unchanged.
...
This preserves P2's durable tab-per-task human-watching decision while changing the container owner to the project, so a captain can distinguish projects and still switch each project's tasks with `ctrl+b <n>`.
...
Tab-per-task within each project's own workspace remains the durable default for the reason P2 originally found: attaching once shows every one of that project's tasks as a tab in one tab bar, switchable with `ctrl+b <n>`, matching how a captain already watches a tmux-backed fleet.
...
The secondmate's `Archon-<secondmate-id>` supervisor workspace follows the same create-or-reuse rule for that secondmate's own tasks.
...
An absent file is off, and the off path runs the existing project-workspace and `fm-<id>`-tab command sequence unchanged.
...
The `kind=secondmate` agent itself always uses its ordinary `Archon-<id>` parent workspace and never receives a corner projection; only eligible crewmates and scouts launched by that home project beneath it.
...
Every newly created child uses the literal U+2514 `└`, one space, the concise task label with redundant `firstmate/`, `2ndmate-<id>/`, `Archon-<id>/`, and presentation-level `fm-` owner prefixes removed, then the unchanged ` · p:<full-22-character-token>` suffix.
...
The new response-derived workspace id is inserted immediately after its owning project Fleet or Archon parent contiguous child block and before the next parent.
...
A confirmed dead or agent-free endpoint may fall back to the normal flat project workspace while the old projected workspace remains untouched.
...
- Clean projected creates form one stable contiguous child block immediately after their owning project Fleet or Archon parent; existing ambiguous or manually interleaved layouts degrade with a warning instead of being rewritten.
...
- `herdr_workspace_id=` - the id of the exact workspace containing this task's endpoint, ordinarily the project's `<name>-Fleet` workspace or a secondmate's own `Archon-<id>` workspace, and a disposable task workspace when the optional projection succeeds; for reference only, since day-to-day operations use the recorded pane target.
...
Unlike tmux's probe, herdr's has no equivalent "which harness is running under a generic interpreter name" ambiguity: the classification comes from herdr's own registered-agent state, not a process name, so herdr correctly resolves every verified harness including `pi` and OMP. OMP needs no Bun-specific exception on Herdr because the backend reads registered-agent state rather than process names.
...
The focused unit output included these exact label assertions:

```text
ok - fm_backend_herdr_workspace_label: a primary home (no marker) resolves to 'Atlas'
ok - fm_backend_herdr_workspace_label: a secondmate home (.fm-secondmate-home) resolves to 'Themis-<id>'
ok - fm_backend_herdr_workspace_label: an empty marker file falls back to the primary label 'Atlas'
ok - fm_backend_herdr_workspace_label: an unreadable marker file falls back to the primary label 'Atlas'
ok - fm_backend_herdr_workspace_label: two different secondmate homes get two different, non-colliding labels
```

The helper unit suite passed all seven safety cases, including guarded teardown and the fleet-state tripwire.
...
A current real-Herdr run was attempted only through the mandatory `/Users/ed/Developer/Atlas/Themis/bin/fm-herdr-lab.sh` contract, with a generated non-`default` session, an installed teardown trap, and guarded `provision`.
Provisioning stopped before creating a lab session with this exact output:

```text
fm-herdr-lab: fleet-state tripwire requires exactly one running default session
```

A helper-scoped `session list --json` confirmed that the `default` session was present with `"running":false`, while a separate non-default `atlas` session was running.
The tripwire deliberately refuses that state, so the real smoke, workspace-per-home, restart, respawn, and prune-safety E2E suites were not run for this label change.
...
bash tests/fm-backend-herdr-prune-safety-e2e.test.sh
bin/fm-lint.sh
```

The real smoke output confirmed that the primary workspace was created as `Atlas`, the secondmate workspace was created as `Themis-<secondmate-id>`, and both exact labels survived a guarded session restart.
The workspace-per-home E2E confirmed primary-owned and secondmate-owned spawns, home-scoped recovery, and targeted teardown under the new labels.
The restart regression confirmed that an `Atlas` workspace survives restart and replaces restored task husks without changing `fm-<task-id>` tab labels.
The prune-safety E2E confirmed that a pre-existing `Atlas` collision is adopted without pruning its live tab.
Every focused command exited successfully, and the helper teardown tripwire left the default session unchanged.

The bounded baseline command from `.no-mistakes.yaml` was also run twice:

```sh
command -v tmux >/dev/null || { echo "tmux is required for e2e tests" >&2; exit 1; }; tmux -V; rc=0; for t in tests/*.test.sh; do echo "== $t =="; bash "$t" || rc=1; done; exit "$rc"
```

It reported three failures unrelated to the label patch: the tmux smoke expected Bash prompt behavior while the machine supplied Fish, the zellij smoke returned an empty current path, and the secondmate-charter assertion failed on current upstream.
...
The change is confined to the single owner `fm_backend_herdr_workspace_label` (`bin/backends/herdr.sh`), which every create/find/adopt/recovery path already routes through, plus the tests, comments, and docs that assert the labels.

Scope note (disconfirming check): a repo-wide audit found that inside this repository `Atlas` was ONLY ever the herdr workspace label - there is no `Atlas` coding-agent or status-line brand anywhere in the tree, and no `Atlas` agent identity was touched.
The captain's separate `Atlas` agent/status-line brand lives outside this repository and is deliberately unchanged; this relabel is purely the herdr workspace label.

The installed client during verification:

```text
herdr 0.7.4-preview.2026-07-17-813fec141faa, protocol 16
```

Before/after of the label owner, read directly from `fm_backend_herdr_workspace_label` across every home shape (primary, secondmate id `demo-d1`, empty marker, unreadable marker):

```text
before:  primary=Atlas   secondmate=Themis-demo-d1   empty=Atlas   unreadable=Atlas
after:   primary=Themis  secondmate=Archon-demo-d1   empty=Themis  unreadable=Themis
```

A guarded lab reproduction confirmed the live-displayed label end to end before the fix: a workspace created through `bin/fm-herdr-lab.sh run <session> workspace create --label "$(fm_backend_herdr_workspace_label)" --no-focus` for a primary home displayed `Atlas` in `workspace list`, and the fleet-state tripwire showed the running `default` session unchanged after guarded teardown.

The exact commands run for the fix were:
...
The real-Herdr suites ran with a running `default`-session tripwire and passed.
The smoke output confirmed the primary workspace was created as `Themis` and the secondmate workspace as `Archon-<secondmate-id>`, and both exact labels survived a guarded session stop plus restart.
The workspace-per-home E2E confirmed an ordinary crewmate lands in the primary's `Themis` workspace, a `--secondmate` spawn and a crewmate spawned from the secondmate's own process both land in the secondmate's `Archon-<id>` workspace, list-live stayed home-scoped, and targeted teardown closed only its own tab.
The respawn-idempotency E2E confirmed reuse and restart-husk close-and-replace preserve the `Themis` workspace without leaking duplicates, and the prune-safety E2E confirmed a pre-existing `Themis`-labeled collision is adopted without pruning its live tab (the 2026-07-02 self-kill class does not reproduce under the wider-collision `Themis` label).
The lint owner reported this exact version line and exited successfully:
...
This lets a project whose registry KEY differs from its clone-directory basename (the firstmate repo registered as `AgentThemis` at `.../Themis`) resolve its mode and Fleet through one identity, while the basename stays the display default so the differing key never renames a `<basename>-Fleet` workspace without an explicit `fleet=` alias.
...
```text
key 'AgentThemis' (no fleet alias), clone dir basename 'Themis'  -> workspace 'Themis-Fleet'  (no rename)
