...
Herdr uses one tab per task, groups ordinary work in `FM-fleet-<n>` or `SM-fleet-<n>` workspaces, and places secondmate agents as tab `Portside` in the primary workspace (`TheBridge` by default).
