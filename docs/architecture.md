...
Herdr's container shape is workspace-per-home plus tab-per-task: the primary home uses workspace label `Atlas`, secondmate homes use `Themis-<secondmate-id>`, and recovery/list-live scopes to the current `FM_HOME`'s workspace.
