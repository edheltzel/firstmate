...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that same cadence to an unchanged `paused:`, durable `captain-held`, or terminal captain-relevant endpoint such as a completed `done:` task only when the backend confidently reports its agent dead.
A retained pane whose worker has exited on a terminal status - a preserved checkpoint branch, or a completed local-only task awaiting merge approval - expects no further worker action, so its bounded recheck is anchored on the status file rather than the pane hash and cannot be re-triggered by the idle pane redrawing a new hash.
...
Herdr's container shape is workspace-per-project plus tab-per-task: an ordinary worker lands in its project's `<Fleet display name>-Fleet` workspace (the configured alias or the repository name), the persistent secondmate agent keeps its own `Archon-<secondmate-id>` supervisor workspace, and recovery/list-live scopes to the resolved project label.
