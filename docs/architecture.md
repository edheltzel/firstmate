...
Herdr's container shape is workspace-per-home plus tab-per-task: the primary home uses workspace label `Themis`, secondmate homes use `Archon-<secondmate-id>`, and recovery/list-live scopes to the current `FM_HOME`'s workspace.
