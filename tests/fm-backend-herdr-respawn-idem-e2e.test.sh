...
RAW=$(FM_HOME="$SCRATCH" fm_backend_herdr_container_ensure ship "$PROJ_CWD") || fail "container_ensure failed"
CONTAINER=${RAW%%$'\t'*}
SEEDED_TAB_ID=${RAW#*$'\t'}
WSID=${CONTAINER#*:}
WS_LABEL=$(herdr workspace list --session "$SESSION" 2>&1 | jq -r --arg id "$WSID" '.result.workspaces[]? | select(.workspace_id == $id) | .label')
[ "$WS_LABEL" = "FM-fleet-1" ] || fail "the restart regression workspace should use the project worker label 'FM-fleet-1', got '$WS_LABEL'"
pass "repro setup: the restart regression workspace uses the project worker label 'FM-fleet-1'"
...
[ "$RESTARTED_WS_LABEL" = "FM-fleet-1" ] || fail "the project worker label should survive restart exactly, got '$RESTARTED_WS_LABEL'"
