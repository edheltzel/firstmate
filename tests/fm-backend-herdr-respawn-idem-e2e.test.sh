...
[ "$WS_LABEL" = "proj-Fleet" ] || fail "the restart regression workspace should use the project worker label 'proj-Fleet', got '$WS_LABEL'"
pass "repro setup: the restart regression workspace uses the project worker label 'proj-Fleet'"
...
[ "$RESTARTED_WS_LABEL" = "proj-Fleet" ] || fail "the project worker label should survive restart exactly, got '$RESTARTED_WS_LABEL'"
