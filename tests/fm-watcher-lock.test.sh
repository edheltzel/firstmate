...
test_watch_restart_attaches_to_healthy_peer() {
  ...
  PEER_READY_FILE="$peer_ready" node -e 'const fs = require("fs"); process.on("SIGTERM", () => {}); fs.writeFileSync(process.env.PEER_READY_FILE, "ready\\n"); setTimeout(() => {}, 300000)' &
  peer=$!
  i=0
  while [ "$i" -lt 80 ] && [ ! -e "$peer_ready" ]; do
    sleep 0.1
    i=$((i + 1))
  done
  [ -e "$peer_ready" ] || fail "TERM-resistant peer did not publish readiness"
