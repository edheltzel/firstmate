...
The same launch and the same locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/backend`, `config/herdr-presentation-spaces`, `config/startup-memory-budget`, `config/worker-git-identity`, and the one shared captain-preference file `data/captain-shared.md`.
The worker identity file contains only public identity and verification policy; the private signing key remains machine-local and is never copied into a secondmate home.
