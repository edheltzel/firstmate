...
A project can be registered under a key that differs from its checkout directory name - most notably the firstmate repo itself, registered as `AgentThemis` while its checkout basename is `Themis`.
