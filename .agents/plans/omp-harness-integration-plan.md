...
The next and only validation task before activation is `omp-final-authority-redteam-o10`, whose exact report path is `data/omp-final-authority-redteam-o10/report.md`.

O8 and O9 are preserved historical `BLOCK` reviews and cannot authorize activation. Implementation authority is blocked until `omp-final-authority-redteam-o10` returns exact `PASS` with no plan-blocking finding and its decision-hold inventory verifies clean.
...
The existing live activation identity is `omp-p1-activation-a7`, and O10 is its only possible report authority.

The activation task records the captain's implementation authorization dated 2026-07-27, but it must refuse to activate any P1-P8 task unless O10 returns exact `PASS` with no plan-blocking finding.

The current next gate is `omp-final-authority-redteam-o10`, blocked by the completed historical O9 correction result, while O8 and O9 remain completed `BLOCK` results rather than activation authorizations.
...
The independent O10 Red Team must validate the corrected P0 contract after the historical O9 BLOCK and before any future implementation task is activated.
...
The gate must fail closed unless the final authority Red Team report for `omp-final-authority-redteam-o10` at `data/omp-final-authority-redteam-o10/report.md` has an exact `PASS` disposition, contains the exact heading `## Plan-blocking findings` followed by exactly `None.`, and has a verified decision-hold inventory. The O8 and O9 historical reports are never accepted.
...
They become executable Tasks Axi records only after `omp-final-authority-redteam-o10` passes, its decision-hold inventory verifies clean, and `omp-p1-activation-a7` completes its fail-closed checks.
...
| P0 | Historical final corrected-plan validation | `omp-final-corrected-plan-redteam-o9` | `omp-corrected-plan-redteam-o8` | serialized | complete 2026-07-28; preserved `BLOCK`; never an activation authority |
| P0 | Independent final authority Red Team | `omp-final-authority-redteam-o10` | `omp-final-corrected-plan-redteam-o9` | serialized | queued; exact `PASS` with `## Plan-blocking findings` / `None.` is required |
| P1 | Fail-closed implementation activation | `omp-p1-activation-a7` | `omp-final-authority-redteam-o10` | serialized | activation gate only; captain authorization `captain-omp-implementation-authorization-2026-07-27` is a separate gate; no P1-P8 implementation record is active now |
...
O6 is serialized before the correction task, and O8, O9, O10, and activation are serialized before any implementation task is added to the live backlog.

The current O6 result is `BLOCK`, so this plan is not an implementation authority.

`omp-plan-block-corrections-o7` owns the correction artifact, `omp-corrected-plan-redteam-o8` owns the promoted correction after its historical BLOCK, `omp-final-corrected-plan-redteam-o9` owns the historical BLOCK review, and `omp-final-authority-redteam-o10` owns the independent final re-review of every O6 acceptance criterion.

### P1 - Evidence and launch boundary

Prerequisite: O10 returns `PASS`, `omp-p1-activation-a7` completes, and the manifest is activated without adding OMP to verified policy.
...
`bin/fm-omp-activation.sh` is the non-mutating-by-default O10 PASS gate and the sole guarded single-backlog atomic publication owner for the first corrected implementation phase.
...
| O10-ACT-01 | Encode the already-authorized activation behind a mechanical O10 PASS, decision, clean-tree, preimage, support-fence, and no-premature-row gate. | `omp-p1-activation-a7` | `bin/fm-omp-activation.sh --check --json`; `omp-activation-check.v1`; every refusal condition is covered by `tests/fm-omp-activation.test.sh` | `omp-rollback-omp-p1-activation-a7`; default is refusal and `--activate` alone is never authority. |
| O8-TRACK-01 | Replace prose dependency aliases with exact stable task IDs in a machine-readable graph and a parity-checked roadmap. | All task IDs in `.agents/tasks/omp-manifest.json` | `bin/fm-omp-plan-check.sh --json`; `omp-plan-check.v1`; Tasks Axi 0.2.3 list/show/ready | `omp-rollback-omp-corrected-plan-redteam-o8`; no future task row is executable before O10 and A7. |
...
| O8-PROV-01 | Treat O7/O9 branch text as historical and derive current branch and commit from live Git state. | `omp-final-authority-redteam-o10`, `omp-p1-activation-a7` | `git branch --show-current`, `git rev-parse HEAD`, and activation preflight fields | A stale or mismatched preimage refuses activation; no historical branch is authoritative. |
...
| V26 | Current-code owner and artifact-map completeness | REQ-MAP-01 | `omp-plan-block-corrections-o7`, `omp-final-authority-redteam-o10`, `omp-p8-policy-publication`; plan owner | `git ls-files`, exact owner table, and path checks cover secondmate positional parsing, raw launch, generated hook, send, continuity, every teardown list, package/type/load validation, and full regression. | Live-derived path inventory matches the current tree, missing or stale owner paths block O10 and P8, and the artifact/rollback rows exist for every task. |
| V27 | Stale path and terminology closure | REQ-DOC-01 | `omp-plan-block-corrections-o7`, `omp-final-authority-redteam-o10`, `omp-p8-policy-publication`; documentation owner | `legacy_prefix=$(printf 't%s' 'h-'); legacy_role=$(printf 'Ar%s' 'chon'); rg -n "(^|[^[:alnum:]_])$legacy_prefix|persistent[[:space:]]+supervisor|$legacy_role" .agents/plans/omp-harness-integration-plan.md .agents/tasks/roadmap.md .agents/tasks/backlog.md` plus Markdown/link/path checks, with each intentional historical reference classified. | No stale current-surface reference remains, and any individually stale external link is pinned or labeled before publication. |
| V28 | Live Status and Bearings monitoring projection | REQ-MON-01 | `omp-plan-block-corrections-o7`, `omp-final-authority-redteam-o10`, `omp-p8-policy-publication`; reporting owner | Fixture cases for manifest-only, active, blocked, terminal, historical-branch, and genuine decision records run through `bin/fm-omp-monitor-check.sh --json`, `bin/fm-fleet-snapshot.sh --json`, `bin/fm-bearings-snapshot.sh --json`, `tests/fm-status-report.test.sh`, and `tests/fm-omp-monitor-check.test.sh`. | Actual projections show phase, milestone, scoped completed/total, live Git branch, blockers with STOP/owner/containment, next gate, and `needs:human` options only for a genuine decision, without future or historical rows inflating the denominator. |
...
| Plan execution drift | A prose plan can omit a seam or activate tasks early. | Stable manifest, exact semantic projections, phase-scoped progress, O10 validation before activation. | `.agents/tasks/roadmap.md`; progress and blocker fields. |
| Activation drift | O8/O9 completion or captain authorization could be mistaken for implementation permission. | Existing `omp-p1-activation-a7` checks O10 `PASS`, no plan-blocking finding, verified hold, clean diff, and no premature P1-P8 row. | Activation gate; refusal is the safe result and reports the failing check. |
...
| Monitoring drift | Prose progress can disagree with Status or Bearings output or use a historical branch. | V28 fixture projections and live Git branch derivation from `git symbolic-ref` or `git rev-parse`. | `omp-plan-block-corrections-o7`, `omp-final-authority-redteam-o10`; REQ-MON-01. |
...
The next gate is `omp-final-authority-redteam-o10` with explicit `PASS` and no plan-blocking finding required; O9 is historical `BLOCK` evidence only.

The current blocker is the preserved O8/O9 `BLOCK` result and the required independent O10 validation, not an implementation failure.
...
After O10 passes and activation completes, Tasks Axi `blocked-by` edges control readiness and only genuine decisions use structured holds.
...
The `.agents/tasks` roadmap contains stable future IDs as prose, while the parseable backlog contains no executable P1-P8 implementation rows until O10 passes and activation completes.

The live firstmate backlog contains no newly activated P1-P8 implementation task before O10 validation and activation.
