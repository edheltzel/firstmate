...
# OMP 17.2.2 keeps `bun` as tmux's pane_current_command. A verified supervised
# launch showed the pane shell pid's tpgid pointing at `bun bun /.../omp ...`.
# Match only those leading executable arguments; unrelated Bun processes remain
# ambiguous and can never authorize recovery.
...
fm_backend_tmux_agent_state() {  # <target>
  ...
  case "$comm" in
    *claude*|*codex*|*opencode*|*grok*|*kimi*|pi|pi-signed|pi-launcher|Pi) printf 'alive' ;;
    bun) if fm_backend_tmux_omp_foreground_alive "$target"; then printf 'alive'; else printf 'ambiguous'; fi ;;
