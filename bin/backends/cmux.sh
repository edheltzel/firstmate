...
# a session provider ONLY, exactly like herdr/zellij: worktree isolation is
# owned by bin/fm-worktree-lib.sh. Sourced only through bin/fm-backend.sh's
...
# pure session provider (bin/fm-worktree-lib.sh selects the worktree provider)
# and Escape IS natively supported.
#
