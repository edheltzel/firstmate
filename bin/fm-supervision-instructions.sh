...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    pi)
      ...
      ;;
    omp)
      printf '%s%s%s%s%s%s\n' "$prefix" 'repair a missing or failed watcher cycle with the OMP extension tool fm_watch_arm_pi, or restart OMP with -e ' "$pi_turnend_ext" ' -e ' "$pi_ext" ' if the extensions are not loaded.'
      ;;
    opencode)
